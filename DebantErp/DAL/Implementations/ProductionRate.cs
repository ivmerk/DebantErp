...
namespace DebantErp.DAL
{
    public class ProductionRateDAL : IProductionRateDAL
    {
        ...
        {
            string sql =
                "INSERT INTO production_rates (production_operation_id, operation_timeframe) VALUES (@productionOperationId, @operationTimeframe) RETURNING id";
        ...
        {
            string sql =
                "UPDATE production_rates SET production_operation_id = @productionOperationId, operation_timeframe = @operationTimeframe, updated_at = CAST(@UpdatedAt AS timestamp with time zone) WHERE id = @id";
